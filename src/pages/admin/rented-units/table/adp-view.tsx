...
interface ADPViewProps {
  data: IRentedUnit[];
  actions?: (row: IRentedUnit) => React.ReactNode;
  pagination?: {
    currentPage: number;
    pageSize: number;
    totalPages: number;
...
export function ADPView({
  ...
  const tableColumns: ColumnDef<IRentedUnit>[] = actions
    ? [
        ...baseColumns,
        {
          id: "actions",
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const landlordEmail = String(
        row.original.landlordEmail || ""
      ).toLowerCase();

      return (
        unitCode.includes(searchValue) || landlordEmail.includes(searchValue)
      );
    },
