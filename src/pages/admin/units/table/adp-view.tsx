...
interface ADPViewProps {
  data: IAdminUnit[];
  actions?: (row: IAdminUnit) => React.ReactNode;
  pagination?: {
    currentPage: number;
    pageSize: number;
    totalPages: number;
...
export function ADPView({
  ...
  const tableColumns: ColumnDef<IAdminUnit>[] = actions
    ? [
        ...baseColumns,
        {
          id: "actions",
  ...
  if (!tableInitialized) {
    return <LoadingCard Icon={Loader} title="Đang tải danh sách phòng..." />;
  }
      ...
      {data.length > 0 ? (
        ...
        <div className="flex-1">
          <EmptyData
            Icon={Home}
            title="Chưa có phòng!"
            description="Danh sách phòng của bạn đang trống."
          />
        </div>
      )}
    </div>
