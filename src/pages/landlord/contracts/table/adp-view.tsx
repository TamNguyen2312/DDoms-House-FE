...
export function ADPView({
  ...
  const tableColumns: ColumnDef<IContract>[] = actions
    ? [
        ...baseColumns,
        {
          id: "actions",
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      const contractId = String(row.original.id || "").toLowerCase();
      const unitCode = String(row.original.unit?.unitCode || "").toLowerCase();

      // Remove # from search if user types it
  ...
  if (!tableInitialized) {
    return <LoadingCard Icon={Loader} title="Đang tải danh sách..." />;
  }
