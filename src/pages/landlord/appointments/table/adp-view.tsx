...
interface ADPViewProps {
  data: IAppointment[];
  actions?: (row: IAppointment) => React.ReactNode;
  pagination?: {
    currentPage: number;
    pageSize: number;
    totalPages: number;
...
export function ADPView({
  ...
  const tableColumns: ColumnDef<IAppointment>[] = actions
    ? [
        ...baseColumns,
        {
          id: "actions",
      ...
      {data?.length > 0 ? (
        ...
        <div className="flex-1">
          <EmptyData
            ...
            description="Danh sách cuộc hẹn của bạn đang trống. Hãy tạo các cuộc hẹn khi cần thiết."
          />
        </div>
      )}
    </div>
