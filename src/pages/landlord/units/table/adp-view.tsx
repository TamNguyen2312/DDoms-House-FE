...
interface ADPViewProps {
  data: IUnit[];
  actions?: (row: IUnit) => React.ReactNode;
...
export function ADPView({
  ...
  const tableColumns: ColumnDef<IUnit>[] = actions
    ? [
        ...baseColumns,
        {
          id: "actions",
      ...
      {data.length > 0 ? (
        ...
        <div className="flex-1">
          <EmptyData
            ...
            description="Danh sách phòng con của bạn đang trống. Hãy tạo các phòng con để thực hiện cho thuê."
          />
        </div>
      )}
    </div>
